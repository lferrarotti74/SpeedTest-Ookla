...
@test "Container should have speedtest configuration directory" {
    run run_shell_container "test -d /home/speedtest/.config/ookla"
    [ "$status" -eq 0 ]
    print_success "Configuration directory exists"
}

@test "Container should have aliases script" {
    run run_shell_container "test -f /etc/profile.d/aliases.sh"
    [ "$status" -eq 0 ]
...
@test "Container should have proper file permissions" {
    run run_shell_container_output "ls -la /usr/local/bin/speedtest"
    [ "$status" -eq 0 ]
    [[ "$output" =~ "-rwxr-xr-x" ]]
